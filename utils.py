...
def show_compound(dataset: ChainedMNIST, sample: int, chains: list[Chain], compositions: list[Composition], device, name=None) -> None:
  ...
  axarr[0, 0].text(9, width+10*(num_experts//4), 'Canon')

  axarr[0, 1].imshow(trans.reshape(width, width))
  axarr[0, 1].text(5, width+10*(num_experts//4), f'Trans {chain.abbreviation}')
  ...
  for i, comp in enumerate(compositions):
    ...
    axarr[i//2+1, i%2].text(0, width+10*(num_experts//4), f"Recovered {comp.name}")
  if name != None:
...
def init_identity_experts(num_experts, checkpt_dir, model_name, load_initialized_experts, device, train_loader):
  ...
  for expert_index in range(num_experts):
      if load_initialized_experts:
          ...
          expert.optimizer = create_optimizer(expert.parameters(), init_optimizer, lr=init_learning_rate, wd=init_weight_decay)
